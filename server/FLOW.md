...
| Template | `render_template("iaCarry_Local_JS_1_Clouding.html")`, resolved by a `ChoiceLoader`: `server/` first, then the legacy `..\iacarry-evaluation` |
...
Flask   INFO  serving template from: /…/server/iaCarry_Local_JS_1_Clouding.html
...
```bash
python3 server/zz_verify/verify.py     # 71 checks against a stub /upload
```

See `server/zz_verify/README.md`, including what it cannot establish: nothing
