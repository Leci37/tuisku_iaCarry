...
from medium_utils import load_image_into_numpy_array, plot_detections, get_model_detection_function
...
for index_id, row in df_series_mix.iterrows():
    ...
    for index, row in df_p.iterrows():
        x, y, w, h = image_utils.get_scalated_00_x_y_w_h(row)
        x, y, w, h = x, y, (x + w), (y + h)
    ...
    print(index_id , "\t" + image_utils.bcolors.OKBLUE + row['id_mix'] + image_utils.bcolors.ENDC + "\t Count: ", df_p.shape,"\t Num_box: ", len(aux_Gt_boxes) ,"\t Id_labels: ", ', '.join(str(v) for v in aux_Gt_labels) )
