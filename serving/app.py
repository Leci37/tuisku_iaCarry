...
# Directory holding this file (serving/). Its templates/ subfolder is the
# versioned home of the page served at GET /, so the file under version control
# is the file Flask renders. templates/ is also Flask's own default, which is
# why no template_folder argument is needed below.
SERVER_DIR = os.path.dirname(os.path.abspath(__file__))
TEMPLATE_DIR = os.path.join(SERVER_DIR, "templates")
logging.info("TEMPLATE_DIR (primary template folder): " + TEMPLATE_DIR)
...
app = Flask(__name__)
# Templates are looked up in serving/templates/ first, then in the legacy deploy
# folder (..\iacarry-evaluation). Editing the versioned file therefore takes
# effect on reload, while deployments that still copy the template across keep
# working.
from jinja2 import ChoiceLoader, FileSystemLoader
app.jinja_loader = ChoiceLoader([FileSystemLoader(TEMPLATE_DIR), FileSystemLoader(TEMPLATE_FOLDER)])
...
def home_index():
    ...
    try:
        src = app.jinja_env.get_template("iacarry_checkout.html").filename
...
def upload_file():
    ...
    if request.method == "POST":
        ...
        if f and allowed_file(f.filename):
            filename, save_path = server_utils.save_img_loaded(
            ...
            dict_to_respond = server_utils.change_format_dict_json_to_client(
            ...
            jrespon = json.dumps(dict_to_respond, cls=server_utils.NumpyEncoder)
