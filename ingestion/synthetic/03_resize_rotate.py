...
from common.image_utils import reduce_size_for_transparency_size_png, add_img_noBG_from_jpeg, bcolors, avoid_overflow_photo_x_y_w_h
...
#     image_bg = cv2.imread(path)
#     image_bg = cv2.cvtColor(image_bg, cv2.COLOR_BGR2RGB)
#     composition_1, pos_x, pos_y = add_img_noBG(img_background, image_bg, rad_pos_x, rad_pos_y)
#     path_out_reduce_size = path.replace("E:\\iaCarry_img_eroski\\frames_rota\\","E:\\iaCarry_img_eroski\\frames_rota_fon\\")
...
MAX_REDUCTION_LABEL_IMG_PER = 34
MIN_REDUCTION_LABEL_IMG_PER = 10
...
#     path_save = path_load.replace("E:\\iaCarry_img_eroski\\frames_rota_fon\\",
#                                   "E:\\iaCarry_img_eroski\\frames_rota_fon_BBOX\\")
...
PATHS_IMG = glob.glob(r"E:\iaCarry_img_eroski\frames_Mul" + '/*(*).png')
PATHS_IMG = glob.glob(r"E:\iaCarry_img_eroski\frames_Mul" + '/*(*).jpg')
PATHS_IMG = glob.glob(r"E:\iaCarry_img_eroski\frames_Mul" + '/*')
# PATHS_IMG = glob.glob(r"E:\iaCarry_img_eroski\frames_no_bg_bbox\fanta_33cl_0002.png")# + '/*.png')
for path_out_put in PATHS_IMG:
    # os.rename(path_out_put, path_out_put.replace("(", "_").replace(")", ""))
    ...
    path_640= "E:\\iaCarry_img_eroski\\frames_Mul_640\\" + path_key  + ".png"
    image_np_640 = cv2.resize(image_np, (IMAGE_WITHD_BG, IMAGE_HIGNT_BG) )
    cv2.imwrite(path_640, image_np_640)
    print("\t Img 640: ", path_640)

    angle = random.randint(0, 360)
    path_rotated = "E:\\iaCarry_img_eroski\\frames_Mul_640\\" +path_key +"_A" +str(angle) +".png"
    # rotate_bound es que no se recorta la imagen al rotarla
    img_rotated = imutils.rotate(image_np, angle)
    img_rotated = cv2.resize(img_rotated, (IMAGE_WITHD_BG, IMAGE_HIGNT_BG) )
    cv2.imwrite(path_rotated, img_rotated)
    print("\t Rotated img: ", path_rotated)
    # for angle in range(0,360,30):
    #     path__bound = "E:\\iaCarry_img_eroski\\frames_rota\\" +path_key +"_A" +str(angle) +".png"
    #     # rotate_bound es que no se recorta la imagen al rotarla
    #     img_rotated_bound = imutils.rotate_bound(image, angle)
    #     path_rot = "E:\\iaCarry_img_eroski\\frames_rota\\" +path_key +"_B" +str(angle) +".png"
    #     # rotate_bound es que SE recorta la imagen al rotarla
    #     img_rotated_rot  = imutils.rotate(image, angle)
    #
    #     for path_img in [(path__bound,img_rotated_bound ) ,(path_rot,img_rotated_rot ) ]:
    ...
    #
    #         type_prod = re.match(r"(\w*_\w*)_\d{3,5}_[A-Za-z]\d{1,3}[.]png", path.split("\\")[-1] ).groups()[0]
...
# print("\n", r"E:\iaCarry_img_eroski\df_size_Rota_img.csv")
# df.to_csv(r"E:\iaCarry_img_eroski\df_size_Rota_img.csv", sep="\t")
