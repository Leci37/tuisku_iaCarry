...
from common.image_utils import bcolors, get_scalated_00_x_y_w_h
...
count_upload = 0
series_mix = df[:].groupby(["id_mix"])['path'].count()
#   mix_1_00619

for id, num_box in series_mix[450::-1].items():
    df_box = df[df['id_mix'] == id]
    ...
    for index, row_box in df_box.iterrows():
        ...
        x, y, w, h = get_scalated_00_x_y_w_h(row_box)
