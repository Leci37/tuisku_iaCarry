...
def get_img_ram_perspective_transform(img_path, img_path_save):
    ...
    img_p = image_utils.recover_transparency_from_jpg_to_png(img_p, type_img=cv2.COLOR_RGB2RGBA)
    cv2.imwrite(img_path_save, img_p)
    # Necesario reduce_size_for_transparency_size_png pero se hace fuera
    # tu_size_ori, tu_size_red, x0, y0, x1, y1 = image_utils.reduce_size_for_transparency_size_png(img_path_save,img_path_save)  # cv2.cvtColor(img_perspective_transform, cv2.COLOR_BGR2RGB))
    # print(save_name)
...
def generate_img_random_scale(img_np):
    ...
    # tu_size_ori, tu_size_red, x0, y0, x1, y1 = image_utils.reduce_size_for_transparency_size_png(save_name, save_name)
    return img_p
